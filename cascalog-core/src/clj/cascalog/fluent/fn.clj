...
(defn serialize [val]
  (let [type (serialize-type val)
    ...
    (KryoService/serialize {:token (type->token type) :val-ser serialized})))
...
(defmethod serialize-val :var [avar]
  (let [[ns fn-name] (ns-fn-name-pair avar)]
    (KryoService/serialize {:ns ns :fn-name fn-name})))
...
(defmethod serialize-val :serfn [val]
  (let [[env namespace source] ((juxt ::env ::namespace ::source) (meta val))
    ...
    (KryoService/serialize {:ser-meta ser-meta :ser-env ser-env :ns namespace :source source})))
